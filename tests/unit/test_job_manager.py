...
class TestJob:
    ...
    def test_roundtrip(self):
        original = Job(
            job_id="xyz", filename="inv.gds", pdk_name="sky130",
            status=JobStatus.drc_complete, total_violations=42,
        )
...
class TestJobManager:
    ...
    def test_update_status(self, manager):
        ...
        updated = manager.update_status(
            job.job_id, JobStatus.uploaded, gds_path="/tmp/test.gds"
        )
    ...
    def test_update_status_with_error(self, manager):
        job = manager.create("test.gds", "sky130")
        updated = manager.update_status(
            job.job_id, JobStatus.drc_failed, error="KLayout not found"
        )
