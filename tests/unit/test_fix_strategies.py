...
def pdk():
    return PDKConfig(
        ...
        layers={
            ...
            "met2": GDSLayer(
                gds_layer=69, gds_datatype=20, description="Metal 2",
                color="#FF00FF", is_routing=True,
            ),
            "mcon": GDSLayer(
                gds_layer=67, gds_datatype=44, description="Contact",
                color="#808080", is_via=True,
            ),
        },
        rules=[
            DesignRule(
            ...
            DesignRule(
                rule_id="m1.4", rule_type=RuleType.min_enclosure,
                layer="met1", related_layer="mcon", value_um=0.060, severity=5,
            ),
            DesignRule(
...
class TestMinWidthFix:
    ...
    def test_cannot_fix_wrong_type(self):
        ...
        v = Violation(
            ...
            rule_type="min_spacing", severity=6,
        )
    ...
    def test_suggest_width_expansion(self, pdk):
        ...
        suggestion = strategy.suggest_fix(
            violation, violation.geometries[0], pdk, si
        )
        ...
        from backend.core.geometry_utils import polygon_bbox, bbox_width
    ...
    def test_no_fix_when_already_wide(self, pdk):
        ...
        v = Violation(
            ...
            rule_type="min_width", severity=7, value_um=0.140,
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge_pair,
...
class TestMinSpacingFix:
    ...
    def test_suggest_move_fix(self, pdk):
        ...
        violation = Violation(
            category="m1.2", description="spacing", cell_name="TOP",
            rule_type="min_spacing", severity=6, value_um=0.140,
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge_pair,
                    edge_pair=EdgePair(
        ...
        violation = Violation(
            ...
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge_pair,
                    edge_pair=EdgePair(
                        edge1_start=(0, 0), edge1_end=(0, 1),
                        edge2_start=(0.20, 0), edge2_end=(0.20, 1),
                    ),
                )
            ],
        )
...
class TestEnclosureFix:
    ...
    def test_suggest_enclosure_extension(self, pdk):
        ...
        violation = Violation(
            category="m1.4", description="enclosure", cell_name="TOP",
            rule_type="min_enclosure", severity=5, value_um=0.060,
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge_pair,
                    edge_pair=EdgePair(
                        edge1_start=(0, 0.02), edge1_end=(0, 0.18),
                        edge2_start=(0.02, 0.02), edge2_end=(0.02, 0.18),
                    ),
                )
            ],
        )
...
class TestMinAreaFix:
    ...
    def test_suggest_area_extension(self, pdk):
        ...
        violation = Violation(
            category="m1.6", description="area", cell_name="TOP",
            rule_type="min_area", severity=4, value_um=0.083,
            geometries=[
                ViolationGeometry(
...
class TestShortCircuitFix:
    def test_can_fix(self):
        ...
        v = Violation(
            category="met1.short", description="short", cell_name="TOP",
            rule_type="short", severity=10,
        )
        assert strategy.can_fix(v) is True
    ...
    def test_can_fix_by_category_name(self):
        strategy = ShortCircuitFix()
        v = Violation(
            category="short_m1", description="short", cell_name="TOP",
            rule_type=None, severity=10,
        )
        assert strategy.can_fix(v) is True
...
class TestOffGridFix:
    ...
    def test_suggest_snap_to_grid(self, pdk):
        ...
        violation = Violation(
            category="offgrid", description="off-grid", cell_name="TOP",
            rule_type="off_grid", severity=8,
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge,
                    edge_pair=EdgePair(
                        edge1_start=(0.003, 0), edge1_end=(0.003, 0.20),
                        edge2_start=(0.003, 0), edge2_end=(0.003, 0.20),
                    ),
                )
            ],
        )
        ...
        violation = Violation(
            ...
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge,
                    edge_pair=EdgePair(
                        edge1_start=(0, 0), edge1_end=(0, 0.20),
                        edge2_start=(0, 0), edge2_end=(0, 0.20),
                    ),
                )
            ],
        )
        ...
        violation = Violation(
            ...
            geometries=[
                ViolationGeometry(
                    geometry_type=GeometryType.edge,
                    edge_pair=EdgePair(
                        edge1_start=(0.003, 0.003), edge1_end=(0.003, 0.20),
                        edge2_start=(0.003, 0.003), edge2_end=(0.003, 0.20),
                    ),
                )
            ],
        )
